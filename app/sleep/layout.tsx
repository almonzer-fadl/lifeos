...
const subNav = [
  { label: "Dashboard", href: "/sleep" },
  { label: "Trends", href: "/sleep/trends" },
];
    ...
    <PageTransition>
      <div className="sticky top-0 z-30 border-b border-[var(--border)] bg-[rgba(3,4,5,0.92)] backdrop-blur-xl">
        <div className="flex items-center gap-0.5 overflow-x-auto px-3 py-2 scrollbar-none sm:px-4 lg:px-6">
          {subNav.map((item) => {
            const active = item.href === "/sleep" ? pathname === "/sleep" : pathname.startsWith(item.href);
            return (
      ...
      {children}
      <Fab href="/sleep/log" icon="M12 4v16m8-8H4" label="Log Sleep" />
    </PageTransition>
  );
}
