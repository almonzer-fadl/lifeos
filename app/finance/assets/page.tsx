  ...
  return (
    <div className="premium-page">
      <div className="premium-header animate-fade-in flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="premium-kicker">Capital Holdings</div>
          <h1 className="premium-title">Assets</h1>
          <p className="premium-subtitle">{assets.length} holdings tracked</p>
        </div>
        <div className="text-right">
          <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--text-tertiary)]">Total Value</div>
          <div className="font-mono text-2xl font-semibold text-[var(--accent)]">{totalValue.toFixed(0)}</div>
          <div className="text-[10px] text-[var(--text-tertiary)]">Gain {(totalValue - totalCost).toFixed(0)}</div>
        </div>
      </div>
      ...
      {assets.length === 0 ? (
        <section className="premium-panel animate-fade-in">
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
            ...
            <h3 className="mb-1 text-sm font-semibold text-[var(--text)]">No assets tracked</h3>
            <p className="mb-4 max-w-xs text-xs text-[var(--text-tertiary)]">Track investments, property, vehicles, and other holdings.</p>
            <Link href="/finance/assets/new" className="premium-action">Add Asset</Link>
          </div>
        </section>
      ) : (
        <section className="premium-panel animate-fade-in">
          <div className="space-y-2">
            {assets.map((a) => {
              const pnl = a.currentValue - a.purchaseValue;
              return (
                <Link
                  key={a.id}
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 flex items-center gap-2">
                      ...
                      <span className="text-xs text-[var(--text-tertiary)]">{format(new Date(a.createdAt), "MMM yyyy")}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-mono text-sm font-semibold text-[var(--accent)]">{a.currentValue.toFixed(0)} {a.currency}</div>
                    <div className={`text-[10px] font-mono ${pnl >= 0 ? "text-[var(--emerald)]" : "text-[var(--rose)]"}`}>
                      {pnl >= 0 ? "+" : ""}{pnl.toFixed(0)}
                    </div>
                  </div>
