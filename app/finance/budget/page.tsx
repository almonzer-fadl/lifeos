...
export default async function BudgetPage() {
  ...
  return (
    <div className="premium-page">
      <div className="premium-header animate-fade-in"><div className="premium-kicker">{format(new Date(), "MMMM yyyy")}</div><h1 className="premium-title">Budget</h1><p className="premium-subtitle">Monthly income and expense overview</p></div>
      <div className="grid grid-cols-3 gap-2">
        <div className="premium-stat"><div className="premium-label">Income</div><div className="premium-value text-[var(--emerald)]">+{s$(income)}</div></div>
        <div className="premium-stat"><div className="premium-label">Expenses</div><div className="premium-value text-[var(--rose)]">-{s$(expenses)}</div></div>
        <div className="premium-stat"><div className="premium-label">Remaining</div><div className={`premium-value ${remainder >= 0 ? "text-[var(--emerald)]" : "text-[var(--rose)]"}`}>{remainder >= 0 ? "+" : ""}{s$(remainder)}</div></div>
      </div>
            ...
            {Object.entries(byCat).sort(([, a], [, b]) => b - a).map(([name, amt]) => (
              <div key={name} className="space-y-1">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono text-[var(--rose)]">{s$(amt)}</span>
                </div>
      ...
      <section className="premium-panel animate-fade-in">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <div className="space-y-1">{expCats.map((c) => <div key={c.id} className="flex items-center justify-between rounded px-2 py-1.5 text-sm hover:bg-[var(--surface-hover)]"><span className="text-[var(--text-secondary)]">{c.name}</span><span className="font-mono text-xs text-[var(--text-tertiary)]">{byCat[c.name] ? s$(byCat[c.name]) : "0"}</span></div>)}</div>
          </div>
          <div>
