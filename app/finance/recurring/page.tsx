...
export default async function RecurringPage() {
  const recurring = await db.recurringTransaction.findMany({ where: { isActive: true }, orderBy: { nextDate: "asc" }, include: { account: true, category: true } });
  ...
  return (
    <div className="premium-page">
      <div className="premium-header animate-fade-in flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div><div className="premium-kicker">Forward Obligations</div><h1 className="premium-title">Recurring Bills</h1><p className="premium-subtitle">{recurring.length} scheduled payments</p></div>
        <div className="text-right"><div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--text-tertiary)]">Monthly Outflow</div><div className="font-mono text-2xl font-semibold text-[var(--amber)]">{s$(monthlyTotal)}</div></div>
      </div>
      {recurring.length === 0 ? (
          ...
          <Section title="Upcoming (30 Days)" kicker={`${upcoming.length}`}>
            <div className="space-y-1">{upcoming.map((r) => (
              <div key={r.id} className="flex items-center justify-between gap-4 rounded-lg px-4 py-3 transition-colors hover:bg-[var(--surface-hover)]">
                ...
                <span className={`shrink-0 font-mono text-sm font-semibold ${r.type === "expense" ? "text-[var(--rose)]" : "text-[var(--emerald)]"}`}>{r.type === "expense" ? "-" : "+"}{s$(r.amount)} {r.currency}</span>
              </div>
            ))}</div>
          </Section>
          {later.length > 0 && (
