...
const subNav = [
  { label: "Feed", href: "/activity" },
  { label: "Log", href: "/activity/log" },
  { label: "Workouts", href: "/activity/workouts" },
];

const fabMap: Record<string, { href: string; icon: string; label: string }> = {
  "/activity": { href: "/activity/log", icon: "M12 4v16m8-8H4", label: "Log Activity" },
  "/activity/log": { href: "/activity/log/cardio", icon: "M12 4v16m8-8H4", label: "Quick Cardio" },
  "/activity/workouts": { href: "/activity/log/gym", icon: "M12 4v16m8-8H4", label: "Log Workout" },
};

function getFab(pathname: string) {
  for (const [prefix, fab] of Object.entries(fabMap)) {
    if (pathname === prefix || (prefix !== "/activity" && pathname.startsWith(prefix))) {
      return fab;
    }
  }
  return fabMap["/activity"];
}
