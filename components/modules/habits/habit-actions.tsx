...
export function HabitCard({ habit }: { habit: Habit }) {
  ...
  useEffect(() => {
    if (isDoneRef.current !== isDone && !busy) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (format(new Date(sorted[i].date), "yyyy-MM-dd") === format(d, "yyyy-MM-dd")) baseStreak++;
    else break;
  }
  const streak = optimisticDone ? baseStreak + (isDone ? 0 : 1) : baseStreak - (isDone ? 1 : 0);
  ...
  return (
    <>
    <div className={`flex items-center gap-3 rounded-lg border p-4 transition-all ${optimisticDone ? "border-[rgba(66,211,146,0.3)] bg-[var(--emerald-soft)]" : "border-[var(--border)] bg-[var(--surface-raised)] shadow-[var(--shadow-card)]"} ${busy ? "opacity-50" : ""}`}>
      <button onClick={toggle} disabled={busy} className={`h-7 w-7 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${optimisticDone ? "bg-[var(--emerald)] border-[var(--emerald)]" : "border-[var(--border-strong)] hover:border-[var(--emerald)]"}`}>
        {optimisticDone && <svg className="h-4 w-4 text-[var(--bg)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
      </button>
      <div className="flex-1 min-w-0">
        <div className={`text-sm font-semibold ${optimisticDone ? "text-[var(--emerald)]" : "text-[var(--text)]"}`}>{habit.name}</div>
      ...
      <div className="text-right shrink-0">
        <div className="text-lg font-bold text-[var(--text)] font-mono">{displayStreak > 0 ? displayStreak : "—"}</div>
